...
void InitialiseWorldModels() {
	...
	/*
	// Signs (gap between signs = 700)
	InitialiseModel(&sign1, 1900, -200, -1400, 0, 0, 0, (long*)SIGN_1_MEM_ADDR);
	InitialiseModel(&sign2, 1800, -200,  -700, 0, 0, 0, (long*)SIGN_1_MEM_ADDR);
	
	
	
	InitialiseModel(&sign3, 2090, -200, 1480, 0, 0, 0, (long*)SIGN_1_MEM_ADDR);
	RotModel(&sign3.gsObjectCoord, &sign3.rotation, 0, -750, 0);
	
	
	InitialiseModel(&sign4, 1780, -200, 1820, 0, 0, 0, (long*)SIGN_1_MEM_ADDR);
	RotModel(&sign4.gsObjectCoord, &sign4.rotation, 0, -600, 0);
	
	
	InitialiseModel(&sign5, 1530, -200, 2230, 0, 0, 0, (long*)SIGN_1_MEM_ADDR);
	RotModel(&sign5.gsObjectCoord, &sign5.rotation, 0, -450, 0);
	
	
	InitialiseModel(&sign6, 1350, -200, 2700, 0, 0, 0, (long*)SIGN_1_MEM_ADDR);
	RotModel(&sign6.gsObjectCoord, &sign6.rotation, 0, -300, 0);
	
	InitialiseModel(&sign7, 1240, -200, 3230, 0, 0, 0, (long*)SIGN_1_MEM_ADDR);
	...
	*/
	...
	//RotModel(&sign1.gsObjectCoord, &sign1.rotation, 0, -1050, 0);
...
// Approximate 2D distance without sqrt using octagonal approximation (~99% accurate).
...
// which accurately matches the visual shape without the invisible-wall problem of spheres.
...
void CheckWorldCollisions(PlayerStruct *player, long *lateralSpeed) {
	// Sphere collision for buildings and grandstands
	...
	// Adjacent capsule end-caps meet exactly (spacing 1950 = 2 x halfLen), so no gaps.
	...
	// Note: barrier17 was initialised twice; second init (Z=12850) is the live one.
